...
class EasybookReloadedHelper
{
    ...
    private static function getDonationCodeStatus($host, $donation_code)
    {
        if(!empty($host) AND !empty($donation_code))
        {
            ...
            $content = $jhttp_factory->get('https://joomla-extensions.kubik-rubik.de/scripts/je_kr_donation_code_check/je_kr_check_code.php?key='.rawurlencode($donation_code).'&host='.rawurlencode($host));
