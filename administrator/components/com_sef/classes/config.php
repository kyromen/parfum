...
class SEFConfig
{
    ...
    /**
     * Global accept variables
     *
     * @var array
     */
    public $globalAcceptVars = array('Itemid', 'option', 'format', 'layout', 'tmpl', 'lang');
    /**
    ...
    /**
     * Returns the global variable filters
     */
    public function getGlobalFilters()
    {
        ...
        if (!isset($filters)) {
            $numFilter = new stdClass();
            $numFilter->rule = '^[0-9]*$';
            $numFilter->vars = array('Itemid');

            $alphaFilter = new stdClass();
            $alphaFilter->rule = '^[a-zA-Z0-9:._-]*$';
            $alphaFilter->vars = array('format', 'layout', 'tmpl', 'lang');

            $optionFilter = new stdClass();
            $optionFilter->rule = '^com_[a-zA-Z0-9_-]+$';
            $optionFilter->vars = array('option');

            $filters = array('pos' => array($numFilter, $alphaFilter, $optionFilter), 'neg' => array());
        }
