...
class SEFTools
{
    ...
    function &getExtAcceptVars($option, $includeGlobal = true)
    {
        ...
        if( !isset($acceptVars[$option]) ) {
            ...
            if( $aVars == '' ) {
                if ($includeGlobal) {
                    $acceptVars[$option] = $sefConfig->globalAcceptVars;
                }
                else {
                ...
                if ($includeGlobal) {
                    $acceptVars[$option] = array_merge($sefConfig->globalAcceptVars, $aVars);
                }
                else {
    ...
    function &getExtFilters($option, $includeGlobal = true)
    {
        ...
        if( !isset($filters[$option]) ) {
            if ($includeGlobal) {
                $sefConfig = SEFConfig::getConfig();
                $filters[$option] = $sefConfig->getGlobalFilters();
            }
            else {
    ...
    function GetSEFGlobalMeta() {
        return 'a7a45b8d2eda49b1dfabf6dc695b57bd'; // sef.global.meta
    }
     ...
     * @return object
     */
    function PostRequestFollowRedirects($url, $referer = null, $_data = null, $method = 'post', $userAgent = null, $headers = null, $maxRedirs = 5)
    {
        $response = false;
