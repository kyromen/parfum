...
class com_sefInstallerScript
{
    function preflight($action, $parent) {
        ...
        // Check if compatible with previous version
        if ($action == 'update') {
            if (!class_exists('SEFConfig')) {
                require_once(JPATH_ADMINISTRATOR.'/components/com_sef/classes/config.php');
            }
            $sefConfig = SEFConfig::getConfig();
            include ('admin/helpers/artio-update.php');
            $errMsg = '';
            if (!AUpdateHelper::isCompatible('com_joomsef4', $sefConfig->artioDownloadId, $errMsg)) {
                JError::raiseWarning(100, JText::_('COM_SEF_'.$errMsg));
                return false;
            }
        }
