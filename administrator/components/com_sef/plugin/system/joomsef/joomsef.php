...
class plgSystemJoomsef extends JPlugin
{
    ...
    function _wordLinks(&$text)
    {
        ...
        foreach ($words as $word) {
            // Skip current URL so the page doesn't link to itself
            if (trim($word->sefurl, '/') == $curPath) {
                continue;
            }
        ...
        foreach ($list as $word => $urls) {
            ...
            // Replace the word
            // 10.11.2012 dajo: (?<!\pL) and (?!\pL) are used instead of \b for word boundaries to work with UTF-8
            // however PCRE must be compiled with unicode support for this to work (hopefully won't cause problems)
