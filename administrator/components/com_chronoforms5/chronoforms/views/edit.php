...
<?php
	...
	$this->Toolbar->addButton('apply', r_('index.php?ext=chronoforms&act=save&save_act=apply'), l_('CF_APPLY'), $this->Assets->image('apply', 'toolbar/'), 'validate_form');
	$this->Toolbar->addButton('save', r_('index.php?ext=chronoforms&act=save'), l_('CF_SAVE'), $this->Assets->image('save', 'toolbar/'), 'validate_form');
...
function addFieldEvent(elem, button_id){
	...
	//var last_config_event = jQuery('#'+button_id).closest('.gform-line-tr').prev();
	var event_count = jQuery(elem).closest('.form-group').parent().find('.form-group').length - 1;
	jQuery(elem).closest('.form-group').before(last_event.clone().wrap('<p>').parent().html().replace(/\[events\]\[[0-9]+\]/g, '[events]['+event_count+']'));
	//clone one in the real config div // that was with jquery ui
	//jQuery('#'+button_id).closest('.form-group').before(last_event.clone().wrap('<p>').parent().html().replace(/\[events\]\[[0-9]+\]/g, '[events]['+event_count+']'));
}
	...
	jQuery(document).ready(function($) {
		...
		function addLinks(Element){
			//Element.on('mouseenter', function(){
				...
				Element.find('.panel-heading').append(field_icons);
				Element.find('.panel-heading').append('<div class="clearfix"></div>');
			...
			<div class="panel-body">
				<div class="tab-content">
					<div id="general" class="tab-pane active">
						...
						<?php echo $this->Html->formLine('Form[params][theme]', array('type' => 'dropdown', 'id' => 'cform_bootstrap', 'label' => l_('CF_FORM_THEME'), 'values' => 'bootstrap3', 'options' => array('gcoreui' => l_('GCoreUI'), 'bootstrap3' => l_('Bootstrap3')/*, 'semantic1' => l_('Semantic 1')*/), 'sublabel' => l_('CF_FORM_THEME_DESC'))); ?>
														...
														if($wizard_field['type'] == 'container'){
															...
															echo '
																<div class="panel-heading">
																	...
																	<div id="field_icons_'.$k.'" class="field_icons pull-right"><span class="collapse_icon collapse_container_icon action_icon label label-default" title="'.l_('CF_COLLAPSE').'">'.l_('CF_COLLAPSE').'</span><span class="edit_icon edit_container_icon action_icon label label-primary" title="'.l_('CF_EDIT').'">'.l_('CF_EDIT').'</span><span class="drag_icon action_icon label label-warning" title="'.l_('CF_DRAG').'">'.l_('CF_DRAG').'</span><span class="delete_icon delete_container_icon action_icon label label-danger" title="'.l_('CF_DELETE').'">'.l_('CF_DELETE').'</span></div>
					...
					<div id="db_settings" class="tab-pane">
						...
						<?php foreach($tables as $table): ?>
							<?php echo $this->Html->formLine('Form[extras][db_fields_list]['.$table.']', array('type' => 'textarea', 'label' => sprintf(l_('CF_DB_FIELDS_LIST'), $table), 'style' => 'width:auto;', 'rows' => 7, 'cols' => 100, 'sublabel' => l_('CF_DB_FIELDS_LIST_DESC'))); ?>
						<?php endforeach; ?>
						<?php echo $this->Html->formSecEnd(); ?>
						<?php echo $this->Html->formEnd(); ?>
					</div>
