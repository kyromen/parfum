 ...
 * @version $Id: product.php 8615 2014-12-04 13:56:26Z Milbo $
 */
		...
		if (count ($fields) > 1) {
			...
			foreach ($fields as $field) {
				if ($field != $orderby) {
					...
					$orderByLink .= '<div><a title="' . $text . ' ' . $orderDirTxt . '" href="' . $link . '">' . $text . ' ' . $orderDirTxt . '</a></div>';
				}
			}
			$orderByLink .= '</div>';
		}
...
function lowStockWarningEmail($virtuemart_product_id) {
	...
	function getProductsByManufactureIndex ($index, $virtuemart_category_id) {
		$object = 2;
		$select =  " pm.`virtuemart_product_id` FROM `#__virtuemart_product_manufacturers` as pm ";
		$joinedTables = " LEFT JOIN `#__virtuemart_product_categories` as c ON c.`virtuemart_product_id` = pm.`virtuemart_product_id` LEFT JOIN `#__virtuemart_products` as p ON p.`virtuemart_product_id` = pm.`virtuemart_product_id` ";
		$whereString = " WHERE p.`published` =1 AND (c.`virtuemart_category_id` = " . (int)$virtuemart_category_id . " OR c.`virtuemart_category_id` IN (SELECT `category_child_id` FROM `#__virtuemart_category_categories` WHERE `category_parent_id` = " . (int)$virtuemart_category_id . ")) AND pm.`virtuemart_manufacturer_id` IN (SELECT `virtuemart_manufacturer_id` FROM `#__virtuemart_manufacturers_ru_ru` WHERE `mf_name` REGEXP \"^" . $index . "\") ";
		$groupBy = '';
		$orderBy = '';
		$filter_order_Dir = '';
		$nbrReturnProducts = false;
		return $this->exeSortSearchListQuery($object, $select, $joinedTables, $whereString, $groupBy, $orderBy, $filter_order_Dir, $nbrReturnProducts);
	}
}
// No closing tag