                ...
                protected function getInput()
                {
                        ...
                        if ($curl_enabled && $allow_url_fopen)
                        {
                                ...
                                $field = '<div id="optimize-images-container" >'
                                        . '<div id="file-tree-container" style="max-width:350px;float:left;"></div>';
                        ...
                        else
                        {
                                ...
                                $message = !$allow_url_fopen ? JText::_('The PHP setting \'allow_url_fopen\' must be enabled for this feature') : '';
                                $message = !$curl_enabled ? JText::_('You must have cURL enabled on your server for this feature to work.'): $message;
		...
		<p>' . $message . '</p>
                ...
                protected function optimizeImages()
                {
                        $arr = JFactory::getApplication()->input->getArray(
                                array('dir' => 'string', 'cnt' => 'int', 'status' => 'string', 'msg' => 'raw'));

                        $oController = new JControllerLegacy();

                        if ($arr['status'] == 'fail')
                        {
                                $oController->setMessage(JText::_('Try again. The optimize image function failed with message "' . $arr['msg'] . '"'),
                                                                          'error');
                        }
                        else
                        {
