...
$Id: cartview.html.php 28 2015-01-25 15:33:38Z abhishekdas $
----------------------------------------------------------------------------------------------------------*/
	...
	private function getMessages($ignoreAddress = true)
	{
		...
		if(!empty($msgs))
		{
			...
			foreach($msgs as $msg) 
			{
				...
				$messages[] = str_replace(array('<br/>', '<br />'), '', $msg['message']);
			}
			$messages = !empty($messages) ? implode('<br/>', $messages) : '';
		}
		
		$messages = trim(strval($messages));
		return $messages;
	}
						...
						foreach($methods as $key => &$method)
						{
							if(strpos($payment, 'id="payment_id_' . $method->virtuemart_paymentmethod_id . '"') !== false)
							{
								...
								if(strpos($payment, 'vmpayment_cardinfo') !== false)
								{
									...
									{
										$payment = str_replace('vmpayment_cardinfo', 'vmpayment_cardinfo additional-payment-info ' . $method->payment_element . $method->payment_params->get('paypalproduct', '') . ' soft-show', $payment);
									}
									else
									{
										$payment = str_replace('vmpayment_cardinfo', 'vmpayment_cardinfo additional-payment-info ' . $method->payment_element . $method->payment_params->get('paypalproduct', '') . ' soft-hide', $payment);
									}
								}
