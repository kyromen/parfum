/*--------------------------------------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------------------------------------
 $Revision: 31 $
 $LastChangedDate: 2015-01-26 21:08:42 +0530 (Mon, 26 Jan 2015) $
 $Id: plugin-min.js 31 2015-01-26 15:38:42Z abhishekdas $
 ----------------------------------------------------------------------------------------------------------*/
VPOPC = window.VPOPC || {};
VPOPC.option = {};
    ...
    var ProOPC = {
        ajaxPool: {}, spinnervars: function () {
            ...
            if (e == "guest") {
                ...
                jq(".proopc-reg-advantages, .proopc-guest-form").show().animate({opacity: 1}, 500);
                ProOPC.inputwidth();
                ProOPC.selectwidth()
            } else {
                jq(".proopc-reg-form").show().animate({opacity: 1}, 500, function () {
            ...
            jq.ajax({
                type: "POST", url: VPOPC.option.URI, data: jq("#GuestUser").serialize(), beforeSend: function () {
            ...
            jq.ajax({
                ...
                }, url: VPOPC.option.URI, data: jq("#UserRegistration").serialize() + e, success: function (e) {
                    if (e.msg) {
                        if (e.error == 1) {
            ...
            if (e.error == 0) {
                ...
                jq.ajax({
                    url: VPOPC.option.URI, data: "task=procheckout&" + t, dataType: "html", success: function (e) {
                ...
                jq("html,body").animate({scrollTop: jq("#proopc-system-message").offset().top - 100}, 500);
                jq("#proopc-system-message").children("span").animate({opacity: 0}, 2e3)
            }
        }, loginAjax: function () {
            ...
            jq.ajax({
                ...
                }, url: VPOPC.option.URI, data: jq("form#UserLogin").serialize(), success: function (e, t, n) {
                    if (e == "1" || e == 1) {
            ...
            if (n == r) {
                ...
                jq.ajax({
                    ...
                    }, dataType: "JSON", url: VPOPC.option.URI, data: i, success: function (e) {
            ...
            jq("#proopc-page-overlay").css({display: "block", height: e}).animate({opacity: .7}, 300);
            ...
            jq(".hover-tootip").hover(function () {
                ...
                var t = e.pageX + 20;
                var n = e.pageY + 10;
                jq(".proopc-tooltip").css({top: n, left: t})
            })
        }, fieldTip: function () {
            ...
            var l = s.top - f;
            var c = s.left + (i - a) / 2;
            o.css({top: l, left: c, visibility: ""}).fadeIn(200);
            ...
            jq(r).animate({opacity: 0}, 200, function () {
                jq(this).remove()
            })
        }, style: function () {
            var e = 0;
            ...
            if (typeof window.bonusCartItemIds === "undefined") {
            } else {
            ...
            if (VPOPC.option.STYLERADIOCHEBOX == 1) {
                ...
                jq('#UserRegistration input[type="checkbox"], #EditBTAddres input[type="checkbox"], #EditSTAddres input[type="checkbox"]').each(function () {
                    jq(this).css({"float": "left", margin: "4px 5px 0 0"});
            ...
            if (VPOPC.option.TOSFANCY) {
                var u = jq("div#proopc-tos-fancy"), a = u.parent(), f = jQuery.easing["jswing"] || jQuery.easing["jswing"] != "undefined" ? "elastic" : "fade";
            ...
            if (VPOPC.option.RELOAD && !jq("#ProOPC").hasClass("loaded") && jq(".proopc-reload").length) {
                jq("html,body").animate({scrollTop: jq("#proopc-system-message").offset().top - 100}, 500, function () {
            ...
            if (e) {
                var t = '<div class="proopc-alert proopc-error-msg"><button type="button" class="close" onclick="ProOPC.close(this);">x</button>' + VPOPC.JText._("COM_VIRTUEMART_USER_FORM_MISSING_REQUIRED_JS") + "</div>";
                jq("#proopc-system-message").html(t);
                jq("html,body").animate({scrollTop: jq("#proopc-system-message").offset().top - 100}, 500);
                return false
            }
            if (jq('#proopc-shipments input[name="virtuemart_shipmentmethod_id"]').is(":checked") == false) {
                var t = '<div class="proopc-alert proopc-error-msg"><button type="button" class="close" onclick="ProOPC.close(this);">x</button>' + VPOPC.JText._("COM_VIRTUEMART_CART_NO_SHIPMENT_SELECTED") + "</div>";
                jq("#proopc-system-message").html(t);
                jq("html,body").animate({scrollTop: jq("#proopc-system-message").offset().top - 100}, 500);
                return false
            }
            if (jq('#proopc-payments input[name="virtuemart_paymentmethod_id"]').is(":checked") == false) {
                var t = '<div class="proopc-alert proopc-error-msg"><button type="button" class="close" onclick="ProOPC.close(this);">x</button>' + VPOPC.JText._("COM_VIRTUEMART_CART_NO_PAYMENT_SELECTED") + "</div>";
                jq("#proopc-system-message").html(t);
                jq("html,body").animate({scrollTop: jq("#proopc-system-message").offset().top - 100}, 500);
                return false
            }
            ...
            if (n.hasClass("klarnaPayment")) {
                ...
                jq(s).find("input:visible").not(":checkbox").each(function () {
                    if (jq(this).val() == "") {
                        jq(this).addClass("invalid");
                ...
                jq(s).find('input[type="checkbox"]').each(function () {
                    if (jq(this).is(":checked") == false) {
                        ...
                        i.push("Klarna: " + VPOPC.JText._("COM_VIRTUEMART_CART_PLEASE_ACCEPT_TOS"));
                        r = true
                    }
                });
                var u = "";
                if (jq(i).length > 0) {
                    u = VPOPC.JText._("COM_VIRTUEMART_USER_FORM_MISSING_REQUIRED_JS") + ": " + i.join(", ") + "."
                }
            ...
            var h = a + "&" + f + "&ctask=savebtaddress&stage=final&" + l + "=" + c;
            jq.ajax({
                beforeSend: function () {
                    ProOPC.addpageloader();
                    jq("#proopc-page-spinner").after('<div id="proopc-order-process"></div>');
                    ...
                    jq("#proopc-order-submit").attr("disabled", "disabled")
                }, dataType: "json", type: "POST", url: VPOPC.option.URI, data: h, success: function (e) {
                    if (e.error == 1) {
                        ProOPC.removepageloader();
                        ProOPC.setmsg(1, e.msg);
                        jq("#proopc-order-submit").removeAttr("disabled");
            ...
            jq.ajax({
                ...
                success: function (e, t, n) {
                    if (n.getResponseHeader("content-type").indexOf("text/html") >= 0 && typeof klarna !== "undefined") {
                        ProOPC.removepageloader();
                        jq("#proopc-order-process").remove();
                        jq("<div/>", {id: "proopc-temp", style: "display:none"}).appendTo("body");
            ...
            if (t) {
                ...
                jq(e).parents(".proopc-klarna-payment").find(".klarna_box_bottom:visible").animate({opacity: 1}, 300)
            }
            ...
            n.push({name: "task", value: "setpayment"});
            n.push({name: "view", value: "cart"});
            n.push({name: "klarna_paymentmethod", value: e.next("input").val()});
            var r = jq("<form></form>");
