 ...
 * @version $Id: cart.php 8611 2014-12-03 15:51:50Z Milbo $
 */
...
class VirtueMartCart {
	...
	*/
	...
	function prepareAjaxData($checkAutomaticSelected=true){
		...
		$data->billTotal = $currencyDisplay->priceDisplay( $this->cartPrices['billTotal'],0,1,false, 0);
		$data->dataValidated = $this->_dataValidated ;

		if ($data->totalProduct > 10 and $data->totalProduct < 20) $data->totalProductTxt = JText::sprintf('COM_VIRTUEMART_CART_X_PRODUCTS', $data->totalProduct);
		else if (in_array($data->totalProduct % 10, array(5,6,7,8,9,0)) and ($data->totalProduct != 0)) $data->totalProductTxt = JText::sprintf('COM_VIRTUEMART_CART_X_PRODUCTS', $data->totalProduct);
		else if (in_array($data->totalProduct % 10, array(2,3,4))) $data->totalProductTxt = JText::sprintf('COM_VIRTUEMART_CART_X2_PRODUCTS', $data->totalProduct);
		else if ($data->totalProduct % 10 == 1) $data->totalProductTxt = JText::_('COM_VIRTUEMART_CART_ONE_PRODUCT');
