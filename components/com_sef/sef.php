    ...
    function updateSitemapNext()
    {
        ...
        for( $i = 0; $i < $count; $i++ ) {
            ...
            if (!empty($option)) {
                ...
                if (class_exists($class)) {
                    $ext = new $class();
                    $sitemap = $ext->getSitemapParams($uri);
                    
                    $sm_indexed = (isset($sitemap['indexed']) ? $sitemap['indexed'] : ($sefConfig->sitemap_indexed ? 1 : 0));
                    $sm_date = date('Y-m-d');
                    $sm_frequency = (isset($sitemap['frequency']) ? $sitemap['frequency'] : $sefConfig->sitemap_frequency);
                    $sm_priority = (isset($sitemap['priority']) ? $sitemap['priority'] : $sefConfig->sitemap_priority);
                    
                    $query = "UPDATE `#__sefurls` SET `flag` = '0', `sm_indexed` = '{$sm_indexed}', `sm_date` = '{$sm_date}', `sm_frequency` = '{$sm_frequency}', `sm_priority` = '{$sm_priority}' WHERE `id` = '{$row->id}'";
                    $db->setQuery($query);
                    if(!$db->query()) {
                    	echo json_encode(array('type'=>'error','msg'=>$db->stderr(true)));
                    	jexit();
