...
class JoomSEF
{
    ...
    function _parseRawRoute(&$uri)
    {
        ...
        $failedVars = array();
        if (!JoomSEF::_varFilterTest($uri, $failedVars)) {
            $failedGlobalVars = array_intersect($failedVars, $sefConfig->globalAcceptVars);
            if (count($failedGlobalVars) > 0) {
                return JoomSEF::_get404vars();
            }
    ...
    function _getMenuTitleLang($option, &$lang, $id = null, $string = null)
    {
        ...
        if (isset($string)) {
            ...
            if (!isset($stringCache[$string])) {
                $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `link` = '$string' AND `published` > 0";
                $db->setQuery($query);
        ...
        elseif (isset($id) && $id != 0) {
            ...
            if (!isset($idCache[$id])) {
                $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `id` = '$id' AND `published` > 0";
                $db->setQuery($query);
            ...
            if (!isset($optionCache[$option])) {
                $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `link` = 'index.php?option=$option' AND `published` > 0";
                $db->setQuery($query);
                $optionCache[$option] = $db->loadObject();
                if (!$optionCache[$option]) {
                    // Try to extend the search for any link to component
                    $query = "SELECT {$jfTranslate}`$column` AS `name`, `language` FROM `#__menu` WHERE `link` LIKE 'index.php?option=$option%' AND `published` > 0";
                    $db->setQuery($query);
                    $optionCache[$option] = $db->loadObject();
                    if (!$optionCache[$option])
    ...
    function _saveNewURL($Itemid, &$metadata, $priority, $temploc, $url,$host, $sitemapParams)
    {
        ...
        "VALUES ('".$temploc."', ".$db->quote(html_entity_decode(urldecode($url))).", '$priority'" . $val . $metavals . ", " . $traceinfo . ", '{$sm_indexed}', '{$sm_date}', '{$sm_frequency}', '{$sm_priority}',".$autolock.",'".$host."')";
        $db->setQuery($query);
    ...
    function _varFilterTest(&$uri, &$failedVars)
    {
        ...
        // Check the accept vars if set to
        if (!is_null($option)) {
            $params =& SEFTools::getExtParams($option);
            if ($params->get('acceptStrict', '0') == '1') {
                $acceptVars =& SEFTools::getExtAcceptVars($option);
                $uriVars = array_keys($uri->getQuery(true));
                $extraVars = array_diff($uriVars, $acceptVars);

                if (count($extraVars) > 0) {
                    $failedVars = $extraVars;
                    return false;
                }
            }
        }
        ...
        $filtersByVar = SEFTools::getExtFiltersByVars($option, $sefConfig->useGlobalFilters);
        //$filtersByVar["id"]["pos"][0]='^[0-9]+(:[\w\-_\d]+)?$';
        /*print_r($filtersByVar["id"]["pos"][0]);
        exit;*/
        ...
        foreach ($filtersByVar as $var => $filterTypes) {
            ...
            if (isset($filterTypes['pos']) && count($filterTypes['pos']) > 0) {
                ...
                foreach($filterTypes['pos'] as $filter) {
                    //preg_match dont work for multibyte characters
                    // unfortunately, mb_ereg_match is in non-default extension mbstring,
                    // so we must check for its presence
                    if (function_exists('mb_ereg_match')) {
                        $chk = (mb_ereg_match($filter, $varValue) == true);
                    } else {
                        $chk = (preg_match('/'.str_replace('/', '\/', $filter).'/', $varValue) > 0);
                    }
                    if ($chk) {
                        $ok = true;
                        break;
            ...
            if (isset($filterTypes['neg']) && count($filterTypes['neg']) ==true)  {
                ...
                foreach($filterTypes['neg'] as $filter) {
                    ...
                    // so we must check for its presence
                    if (function_exists('mb_ereg_match')) {
                        $chk = (mb_ereg_match($filter, $varValue) > 0);
                    } else {
                        $chk = (preg_match('/'.str_replace('/', '\/', $filter).'/', $varValue) > 0);
                    }
                    if ($chk) {
    ...
    function CheckAccess()
    {
        if (isset($_GET['query'])) {
            if (strtolower($_GET['query']) == 'ispaid') {
                echo 'true';
                include('../../administrator/components/com_sef/configuration.php');
                if (isset($artioDownloadId)) {
                    echo "\n";
                    echo $artioDownloadId;
                }
                exit();
            }
        }
